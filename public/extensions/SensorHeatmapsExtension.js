...
export class SensorHeatmapsExtension extends UIBaseExtension {
  ...
  getSensorValue(surfaceShadingPoint, sensorType) {
    ...
    const samples = this.dataView.getSamples(
    ...
    const fractionalIndex = findNearestTimestampIndex(
      samples.timestamps,
      this.currentTime,
      true,
    );
    ...
    if (index1 !== index2) {
      const value =
        samples.values[index1] +
        (samples.values[index2] - samples.values[index1]) *
          (fractionalIndex - index1);
  ...
  onToolbarCreated() {
    this.createToolbarButton(
      "iot-heatmaps-btn",
      "IoT Heatmaps",
      "https://img.icons8.com/ios-filled/50/000000/heat-map.png",
    ); // <a href="https://icons8.com/icon/8315/heat-map">Heat Map icon by Icons8</a>
  }
  ...
  async _setupSurfaceShading(model) {
    ...
    const shadingGroup =
      new Autodesk.DataVisualization.Core.SurfaceShadingGroup("iot-heatmap");
    const rooms = new Map();
    for (const [sensorId, sensor] of this.dataView.getSensors().entries()) {
      ...
      if (!rooms.has(sensor.objectId)) {
        const room = new Autodesk.DataVisualization.Core.SurfaceShadingNode(
          sensorId,
          sensor.objectId,
        );
      ...
      room.addPoint(
        new Autodesk.DataVisualization.Core.SurfaceShadingPoint(
          sensorId,
          sensor.location,
          types,
        ),
    ...
    this._surfaceShadingData =
      new Autodesk.DataVisualization.Core.SurfaceShadingData();
    ...
    for (const type of this.dataView.getChannels().keys()) {
      if (this.dataView.getChannels().get(type).color) {
        const color = this.dataView
          .getChannels()
          .get(type)
          .color.map((x) => parseInt("0x" + x.substring(1)));
