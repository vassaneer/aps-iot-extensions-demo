...
viewer.addEventListener(Autodesk.Viewing.GEOMETRY_LOADED_EVENT, async () => {
  ...
  await dataView.init({
    start: DEFAULT_TIMERANGE_START,
    end: DEFAULT_TIMERANGE_END,
  });
  ...
  adjustPanelStyle(viewer.getExtension(SensorListExtensionID).panel, {
    right: "10px",
    top: "10px",
    width: "500px",
    height: "300px",
  });
  adjustPanelStyle(viewer.getExtension(SensorDetailExtensionID).panel, {
    right: "10px",
    top: "320px",
    width: "500px",
    height: "300px",
  });
  adjustPanelStyle(viewer.getExtension(SensorHeatmapsExtensionID).panel, {
    left: "10px",
    top: "320px",
    width: "300px",
    height: "150px",
  ...
  adjustPanelStyle(levelsExt.levelsPanel, {
    left: "10px",
    top: "10px",
    width: "300px",
    height: "300px",
  ...
  viewer.getExtension(SensorListExtensionID).onSensorClicked = (sensorId) =>
    onCurrentSensorChanged(sensorId);
  viewer.getExtension(SensorSpritesExtensionID).onSensorClicked = (sensorId) =>
    onCurrentSensorChanged(sensorId);
  viewer.getExtension(SensorHeatmapsExtensionID).onChannelChanged = (
    channelId,
  ...
  function onLevelChanged({ target, levelIndex }) {
    dataView.floor =
      levelIndex !== undefined ? target.floorData[levelIndex] : null;
    extensions.forEach((ext) => (ext.dataView = dataView));
  }
