...
export class SensorDetailPanel extends Autodesk.Viewing.UI.DockingPanel {
  ...
  updateCharts(sensorId, dataView) {
    ...
    for (const [channelId, channel] of dataView.getChannels().entries()) {
      ...
      canvases.push(
        `<canvas id="sensor-detail-chart-${channelId}" style="position: absolute; left: 0; top: ${top}px; width: 100%; height: ${chartHeight}px;"></canvas>`,
      );
    }
    ...
    for (const [channelId, channel] of dataView.getChannels().entries()) {
      const canvas = document.getElementById(
        `sensor-detail-chart-${channelId}`,
      );
      const samples = dataView.getSamples(sensorId, channelId);
      this._charts.push(
        this._createChart(
          canvas,
          (samples === null || samples === void 0
            ? void 0
            : samples.timestamps) || [],
          (samples === null || samples === void 0 ? void 0 : samples.values) ||
            [],
          channel.min,
          channel.max,
