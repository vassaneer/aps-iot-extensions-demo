import { DEFAULT_TIMERANGE_START, DEFAULT_TIMERANGE_END } from "./config.js";
...
export function initTimeline(
  ...
  return new Promise(function (resolve, reject) {
    let timeslider = new ChronosEtu.TimeSlider(
    ...
    timeslider.on(
      "tscreated",
      debounce((ev) =>
        onTimeRangeChanged(new Date(ev.start), new Date(ev.end)),
      ),
    );
    timeslider.on(
      "tsmodifying",
    ...
    timeslider.on(
      "timemarkerchanged",
      debounce((ev) => onTimeMarkerChanged(new Date(ev.time))),
    );
    timeslider.on("playbackmarkerchanged", (ev) =>
      onTimeMarkerChanged(new Date(ev.time)),
    );
